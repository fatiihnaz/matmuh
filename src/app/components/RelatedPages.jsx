    ...
    <MainCard title="İlgili Sayfalar">
      <nav aria-label="İlgili sayfalar" className="flex flex-col gap-0.5">
        {items.map((item) => (
          ...
          >
            {item.icon && (
              <item.icon className="size-4 shrink-0 text-primary-500/60" />
            )}
            <span className="flex-1">{item.label}</span>
            <ChevronRight className="size-3.5 shrink-0 text-primary-500/20 group-hover:text-secondary-500 group-hover:translate-x-0.5 transition-all" />
          </Link>
        ))}
      </nav>
    </MainCard>
